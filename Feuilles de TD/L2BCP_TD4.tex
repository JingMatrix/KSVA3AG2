...
%-----------------------------------------------------------------------------
\begin{document}
\noindent Université de Toulouse 
\hfill L2 BCP \\
...
\noindent {\bf Exercice 1 : Test du $\chi^2$ sur l'expérience de Mendel}  \\

\noindent La différence de couleur entre deux lignées de pois considérées est contrôlée par un seul gène, ou de manière 
...
\noindent Dans ses expériences, Mendel a obtenu les résultats suivants : \\
...
\begin{enumerate}
\item L'hypothèse : on note $\pi_{CR}$ la proportion de pois [CR] dans la {\it population} de pois, de même $\pi_{Cr}$,
...
\item On note $\Omega$ l'ensemble des échantillons de $n$ pois et $Q : \Omega \longrightarrow \mathbb{R}$ la variable aléatoire définie par
$$
...
\item Si on prend le risque d'erreur $\alpha=0.05$, quel est le seuil critique $K_{0.05}$ défini par $Pr(Q>K_{0.05})=0.05$ ? Calculez la valeur $q$ de $Q$ correspondant à notre échantillon et concluez.\\
\end{enumerate}
\medskip

\noindent {\bf Exercice 2 : Test du $\chi^2$ d'homogénéité} \\

\noindent On souhaite étudier la différence de la répartition des groupes sanguins entre deux populations humaines.
...
\noindent L'hypothèse qu'on va tester est $(H_0)\quad \pi_{A}=\pi_{A}' \quad \pi_{B}=\pi_{B}'\quad \pi_{O}=\pi_{O}'\quad \pi_{AB}=\pi_{AB}'$. \\

\noindent On a déterminé les groupes sanguins des individus de deux échantillons choisis respectivement dans les deux 
populations. 
...
\end{center}
\smallskip
\item Donnez le degré de liberté du test, le seuil critique et la formule de la statistique de test $Q$. 
\item Calculez la valeur $q$ de $Q$ correspondant à notre échantillon et dites si cette étude a mis en évidence une différence significative de la répartition des groupes 
sanguins entre les deux populations.\\
\end{enumerate}

\medskip
 
\noindent {\bf Exercice 3 : Indépendance entre deux variables qualitatives} \\

\noindent On se demande s'il y a un lien entre l'intensité de l'asthme ("Fort", "Moyen" et "Léger") et la présence d'eczéma
...
\noindent Sur un échantillon de 200 enfants asthmatiques, on a observé les données suivantes :\\
\begin{center}
\small
\begin{tabular}{|c||c|c|c||c|}
\hline
...
\begin{enumerate}
\item L'hypothèse qu'on va tester est :  {\it l'intensité de l'asthme et la présence d'eczéma sont deux variables indépendantes}.
Comment peut-on écrire mathématiquement cette hypothèse ?
\item Calculez les effectifs théoriques.
\item Donnez le degré de liberté et le seuil critique correspondant au risque d'erreur $\alpha=0.05$.
\item Calculez la valeur $q$ de $Q$ correspondant à notre échantillon et concluez.\\
\end{enumerate}

\medskip

\noindent {\bf Exercice 4 : Comparaison de $k$ proportions avec $k\geq 2$}  \\

\noindent Quatre hôpitaux appliquent une méthode différente pour traiter la même maladie. 
...
\begin{enumerate}
\item Quelle est l'hypothèse alternative ?

\item Une étude statistique peut être résumée dans 
le tableau suivant :\\
\small
\begin{center}
